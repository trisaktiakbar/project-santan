<template>
  <div class="card shadow mb-4">
    ...
    <div class="card-body pb-4 mb-3 mt-1 pt-5">
      <div>
        <canvas id="chartCurahHujan" class="w-100"></canvas>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: "CardCurahHujanChartComponent",
  ...
  methods: {
    resultData(items) {
      ...
      this.curahHujan.data = [];
      this.curahHujan.labels = [];

      this.last30days.forEach((e) => {
        this.curahHujan.labels.push(e);
        this.curahHujan.data.push(this.logData[e] ? this.logData[e].curahHujan : null);
      });

      this.createChart("chartCurahHujan", "line", this.curahHujan.labels, this.curahHujan.data);
    },
    errorData(error) {
      console.log(error);
    },
