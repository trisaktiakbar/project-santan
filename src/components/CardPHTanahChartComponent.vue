<template>
  <div class="card shadow mb-4">
    ...
    <div class="card-body pb-4 mb-3 mt-1 pt-5">
      <div>
        <canvas id="chartPHTanah" class="w-100"></canvas>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: "CardPHTanahChartComponent",
  ...
  methods: {
    resultData(items) {
      ...
      this.last30days.forEach((e) => {
        this.pHTanah.labels.push(e);
        this.pHTanah.data.push(this.logData[e] ? this.logData[e].pHTanah : null);
      });

      this.createChart("chartPHTanah", "line", this.pHTanah.labels, this.pHTanah.data);
    },
    errorData(error) {
      console.log(error);
    },
